...
`composer require common-gateway/zgw-stuff-bundle:dev-main` or dockerized: `docker-compose exec php composer require common-gateway/zgw-stuff-bundle:dev-main`
...
`php bin/console commongateway:install common-gateway/zgw-stuff-bundle` or dockerized: `docker-compose exec php bin/console commongateway:install common-gateway/zgw-stuff-bundle`
...
5. Update file names and namespace to your fitting 
   ...
   - Open ZgwStuffBundle.php and change the Bundle name and namespace. The namespace should be the same as your package name in composer.json but in CamelCase. So common-gateway/zgw-stuff-bundle becomes CommonGateway/ZGWStuffBundle
   - Rename the /Service and /ActionHandler accordingly (or delete if not used).
   - Rename the /DependencyInjection/ZgwStuffExtension.php to your BundleNameExtension.php
...
2. Open a php file where you want to use a class.
   - Add the correct use statement (example `use CommonGateway\ZgwStuffBundle\Service\ZgwStuffService;`)
   - U can now use your class!

In the common gateway, if you want to use your code when triggered by an event with a action, make sure the class of the action object is set as the handler name including the namespace. For example if I want to use the ZgwStuffService I can set the ZgwStuffHandler as `CommonGateway\ZgwStuffBundle\ActionHandler\ZgwStuffHandler`.
